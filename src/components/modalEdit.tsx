...
interface ModalEditProps{
  ...
  setDataEmployee: React.Dispatch<React.SetStateAction<Employee|null>>
  id:string|undefined,
}
function ModalEdit({dataEmployee,setDataEmployee,id}:ModalEditProps) {
  const [opened, { open, close }] = useDisclosure(false);
  const [active, setActive] = useState(0);

  const handelPostData = () => {
    axios
      .put( `http://localhost:3000/employees/${id}`, dataEmployee)
      .then((res) => {
        console.log(res);
        close()
      })
